...
1. **性能**：Playwright 复测（生产构建、排除外部字体延迟）— 树节点 114 → 22（仅顶层懒渲染）；渲染耗时 < 2s（实测 1.56s）。
   - DOM 节点口径：**< 4000**（实测 3564）。原稿的 <1500 经实施确认无法达成 — 右侧表格 114 行 ≈ 2900 节点，是剩余大头；按设计第 4 节「表格不加分页」的 YAGNI 决策，验收口径调整为 <4000（已达成），树部分 DOM 已从 5133 降至 ~600。
2. **功能**：展开/折叠正常；搜索过滤含父链保留 + 命中分支自动展开 + 清空恢复折叠；树点选 → 表格高亮（双向）；增删改后 `store.invalidate()` 重载正常。
