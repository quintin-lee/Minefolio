...
// Find or create a category row and return its id. Reuses the existing row when
// the (user_id, name, parent_id) unique key already matches, so re-seeding is a
// no-op instead of raising a UNIQUE constraint error.
static int64_t find_or_create_cat(csilk_db_pool_t* pool, int64_t user_id, const char* name, int64_t parent_id, const char* type, const char* asset_type, int sort_order) {
...
static void ensure_default_categories_for_type(csilk_db_pool_t* pool, int64_t user_id, const char* type) {
    if (strcmp(type, "expense") == 0) {
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            int64_t pid = find_or_create_cat(pool, user_id, items[i].parent_name, 0, "expense", items[i].asset_type, items[i].sort_order);
            if (pid > 0) {
                for (int j = 0; items[i].children[j] != NULL; j++) {
                    find_or_create_cat(pool, user_id, items[i].children[j], pid, "expense", items[i].asset_type, j + 1);
                }
            }
        }
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            int64_t pid = find_or_create_cat(pool, user_id, items[i].parent_name, 0, "income", items[i].asset_type, items[i].sort_order);
            if (pid > 0) {
                for (int j = 0; items[i].children[j] != NULL; j++) {
                    find_or_create_cat(pool, user_id, items[i].children[j], pid, "income", items[i].asset_type, j + 1);
                }
            }
        }
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            int64_t pid = find_or_create_cat(pool, user_id, items[i].parent_name, 0, "transaction", items[i].asset_type, items[i].sort_order);
            if (pid > 0) {
                for (int j = 0; items[i].children[j] != NULL; j++) {
                    find_or_create_cat(pool, user_id, items[i].children[j], pid, "transaction", items[i].asset_type, j + 1);
                }
            }
        }
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            int64_t pid = find_or_create_cat(pool, user_id, items[i].parent_name, 0, "asset", items[i].asset_type, items[i].sort_order);
            if (pid > 0) {
                for (int j = 0; items[i].children[j] != NULL; j++) {
                    ...
                    find_or_create_cat(pool, user_id, items[i].children[j], pid, "asset", child_asset_type, j + 1);
                }
            }
        }
    }
...
// Rename legacy flat top-level category names into the current tree names once,
// so existing accounts pick up the new structure without duplicating categories.
// Each UPDATE is guarded so it never collides with a category that already has
// the target name; id references in records are preserved by the rename.
...
/**
 * @brief Seed default category templates (all four types) for a user.
 *        Runs once per user (tracked in category_seed_state): renames legacy
 *        flat category names into the current tree, then inserts missing
 *        parents and children idempotently. Called at user creation and lazily
 *        on the first categories list request.
 */
