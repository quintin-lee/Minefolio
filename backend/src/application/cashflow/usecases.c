...
int cashflow_usecase_list_schedules(void* pool, int64_t user_id,
                                   csilk_json_t** out_list, cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
...
int cashflow_usecase_get_schedule(void* pool, int64_t user_id, int64_t id,
                                 csilk_json_t** out_item, cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
    memset(out_res, 0, sizeof(*out_res));
...
int cashflow_usecase_create_schedule(void* pool, const create_cashflow_cmd_t* cmd,
                                    int64_t* out_id, cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
    memset(out_res, 0, sizeof(*out_res));
    if (!pool || !cmd) {
        out_res->code = 1002;
    ...
    if (cmd->name) snprintf(s.name, sizeof(s.name), "%s", cmd->name);
    if (cmd->flow_type) snprintf(s.flow_type, sizeof(s.flow_type), "%s", cmd->flow_type);
    if (cmd->frequency) snprintf(s.frequency, sizeof(s.frequency), "%s", cmd->frequency);
    if (cmd->start_date) snprintf(s.start_date, sizeof(s.start_date), "%s", cmd->start_date);
    if (cmd->end_date) snprintf(s.end_date, sizeof(s.end_date), "%s", cmd->end_date);
    if (cmd->note) snprintf(s.note, sizeof(s.note), "%s", cmd->note);
...
int cashflow_usecase_update_schedule(void* pool, const update_cashflow_cmd_t* cmd,
...
int cashflow_usecase_delete_schedule(void* pool, int64_t user_id, int64_t id,
                                    cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
    memset(out_res, 0, sizeof(*out_res));
    if (!pool || id <= 0) {
...
int cashflow_usecase_get_calendar(void* pool, const query_calendar_cmd_t* cmd,
                                 csilk_json_t** out_resp, cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
    memset(out_res, 0, sizeof(*out_res));
    ...
    if (year < 2000 || year > 2100) year = tm_now.tm_year + 1900;
    if (month < 1 || month > 12) month = tm_now.tm_mon + 1;
    ...
    if (mf_cashflow_repo_get_actual_events(pool, cmd->user_id, ym, &actual_evs, &actual_count) == 0 && actual_evs) {
    ...
    size_t sch_count = 0;
    if (mf_cashflow_repo_list_active(pool, cmd->user_id, &schedules, &sch_count) == 0 && schedules) {
        for (size_t i = 0; i < sch_count; i++) {
            ...
            int target_day = 0;
            if (mf_cashflow_rule_matches_month(sch->frequency, sch->start_date, sch->end_date, year, month, &target_day)) {
                ...
                csilk_json_add_string(ev, "flow_type", sch->flow_type[0] ? sch->flow_type : "dividend");
                csilk_json_add_number(ev, "amount", exp_amt);
                csilk_json_add_string(ev, "currency", currency_code(&sch->expected_amount.currency));
...
int cashflow_usecase_confirm_income(void* pool, const confirm_cashflow_cmd_t* cmd,
                                   int64_t* out_tx_id, cashflow_usecase_result_t* out_res) {
    if (!out_res) return -1;
    memset(out_res, 0, sizeof(*out_res));
    if (!pool || !cmd) {
        out_res->code = 1002;
    ...
    if (cmd->target_asset_id <= 0 || cmd->amount <= 0.0 || !cmd->date || !cmd->date[0]) {
        out_res->code = 1002;
        snprintf(out_res->message, sizeof(out_res->message), "Missing required fields (target_asset_id, amount, date)");
        return -1;
    }

    char tx_note[256];
    snprintf(tx_note, sizeof(tx_note), "%s %s",
    ...
    if (tx_id <= 0) {
        ...
        out_res->code = 1002;
        snprintf(out_res->message, sizeof(out_res->message), "Failed to create transaction for cash flow income");
        return -1;
    }

    if (balance_apply_delta(db_pool, cmd->target_asset_id, cmd->user_id, cmd->amount,
                            "transaction", tx_id, tx_note) != 0) {
        csilk_db_exec(db_pool, "ROLLBACK");
        out_res->code = 1002;
