...
int asset_usecase_create(void* pool, const create_asset_cmd_t* cmd, asset_usecase_result_t* out_res) {
    ...
    char* asset_type = asset_get_category_type((csilk_db_pool_t*)pool, cmd->user_id, cmd->category_id);

    mf_asset_t asset = {0};
    asset.user_id = cmd->user_id;
    asset.category_id = cmd->category_id;
    snprintf(asset.name, sizeof(asset.name), "%s", cmd->name);
    if (cmd->account_no) snprintf(asset.account_no, sizeof(asset.account_no), "%s", cmd->account_no);
    if (asset_type) snprintf(asset.asset_type, sizeof(asset.asset_type), "%s", asset_type);
    asset.currency = cur;
    if (cmd->note) snprintf(asset.note, sizeof(asset.note), "%s", cmd->note);
    if (cmd->symbol) snprintf(asset.symbol, sizeof(asset.symbol), "%s", cmd->symbol);
    if (cmd->quote_source) snprintf(asset.quote_source, sizeof(asset.quote_source), "%s", cmd->quote_source);
...
int asset_usecase_update(void* pool, const update_asset_cmd_t* cmd, asset_usecase_result_t* out_res) {
    ...
    const char* account_no = cmd->account_no ? cmd->account_no : csilk_json_get_string(cr, "account_no");
    const char* currency = (cmd->currency && cmd->currency[0]) ? cmd->currency : csilk_json_get_string(cr, "currency");
    ...
    const char* quote_source = cmd->quote_source ? cmd->quote_source : csilk_json_get_string(cr, "quote_source");
    ...
    if (cmd->has_net_value || cmd->has_quantity || cmd->has_cost_basis) {
        if (is_investment) {
            ...
            if (delta != 0) {
                balance_apply_delta(db_pool, cmd->id, cmd->user_id, delta, "asset_netvalue", cmd->id, "net_value update");
            }
            asset_update_position(db_pool, cmd->user_id, cmd->id, new_net, new_qty, new_cost);
            asset_update_basic(db_pool, cmd->user_id, cmd->id, name, account_no, new_current, currency, note, symbol, quote_source);
        }
        csilk_json_free(cur);
        out_res->code = 0;
        snprintf(out_res->message, sizeof(out_res->message), "ok");
    ...
    /* 普通资产更新 */
    if (!asset_update_basic(db_pool, cmd->user_id, cmd->id, name, account_no, cmd->current_value, currency, note, symbol, quote_source)) {
        csilk_json_free(cur);
        out_res->code = 1003;
...
int asset_usecase_query(void* pool, const query_asset_filter_t* filter, csilk_json_t** out_list, int64_t* out_total) {
    if (!out_list || !out_total || !filter) return -1;
    *out_list = asset_list((csilk_db_pool_t*)pool, filter->user_id, filter->page, filter->page_size, filter->category_id, out_total);
...
int asset_usecase_query_logs(void* pool, const query_asset_log_filter_t* filter, csilk_json_t** out_list, int64_t* out_total) {
    if (!out_list || !out_total || !filter) return -1;
    *out_list = asset_balance_logs_list((csilk_db_pool_t*)pool, filter->user_id, filter->page, filter->page_size, filter->asset_id, out_total);
    return (*out_list != NULL) ? 0 : -1;
}
...
int asset_usecase_rebuild_single(void* pool, int64_t user_id, int64_t id, csilk_json_t** out_data, asset_usecase_result_t* out_res) {
    ...
    if (is_investment) {
        ...
        quantity_to_string(pos_state.quantity, buf, sizeof(buf));
        csilk_json_add_number(data, "asset_id", (double)id);
        csilk_json_add_string(data, "quantity", buf);
        money_to_string(pos_state.cost_basis, buf, sizeof(buf));
        csilk_json_add_string(data, "cost_basis", buf);
        money_to_string(pos_state.current_value, buf, sizeof(buf));
        csilk_json_add_string(data, "current_value", buf);
        csilk_json_add_number(data, "tx_count", (double)pos_state.tx_count);
    } else {
        ledger_account_state_t acc_state = {0};
        if (ledger_rebuild_account_balance(db_pool, user_id, id, &acc_state) != 0) {
            csilk_json_free(data);
            out_res->code = 500;
            snprintf(out_res->message, sizeof(out_res->message), "账户余额重建失败");
            return -1;
        }
        money_to_string(acc_state.balance, buf, sizeof(buf));
        csilk_json_add_number(data, "asset_id", (double)id);
        csilk_json_add_string(data, "balance", buf);
        csilk_json_add_number(data, "tx_count", (double)acc_state.tx_count);
    }

    if (out_data) *out_data = data;
    else csilk_json_free(data);
