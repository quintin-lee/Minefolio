...
int mf_position_derive_from_ledger(int64_t            asset_id,
    ...
    if (tx_events && tx_count > 0) {
        for (size_t i = 0; i < tx_count; i++) {
            ...
            if (tx->asset_id != asset_id) continue;

            if (tx->type == LEDGER_TX_BUY) {
                if (mf_cost_basis_apply_buy(&out_position->cost_basis, tx->quantity, tx->amount, tx->fee) != 0) {
                    return -1;
                }
            } else if (tx->type == LEDGER_TX_SELL) {
                if (mf_cost_basis_apply_sell(&out_position->cost_basis, &accum_realized, tx->quantity, tx->amount, tx->fee) != 0) {
                    return -1; // Oversell or error
                }
            } else if (tx->type == LEDGER_TX_DIVIDEND) {
                if (mf_cost_basis_apply_dividend(&out_position->cost_basis, &accum_realized, tx->amount) != 0) {
                    return -1;
                }
            }
        }
    ...
    out_position->valuation = mf_valuation_calculate(out_position->quantity, current_price, NULL, "market");
    out_position->pnl = mf_pnl_calculate(out_position->cost_basis.total_cost,
                                         out_position->valuation.market_value,
                                         accum_realized);
    return 0;
}
