 ...
 */
static inline bool mf_asset_is_investment_type_str(const char* type) {
    if (!type || type[0] == '\0') return false;
    return (strcmp(type, "stock") == 0 ||
            strcmp(type, "fund") == 0 ||
            strcmp(type, "bond") == 0 ||
 ...
 * @brief 判断实体是否属于投资类资产
 */
static inline bool mf_asset_is_investment(const mf_asset_t* asset) {
 ...
 */
static inline bool mf_asset_is_liability_type_str(const char* type) {
    if (!type || type[0] == '\0') return false;
    return (strcmp(type, "credit_card") == 0 ||
            strcmp(type, "loan") == 0 ||
            strcmp(type, "other_liability") == 0);
}

static inline bool mf_asset_is_liability(const mf_asset_t* asset) {
    if (!asset) return false;
