...
int
mf_cashflow_rule_validate(const mf_cashflow_schedule_t* s, char* err_buf, size_t err_len)
{
    if (!s) {
        if (err_buf && err_len) {
            snprintf(err_buf, err_len, "Cashflow schedule is NULL");
        }
        return -1;
    }
    if (s->user_id <= 0) {
    ...
    if (s->source_asset_id <= 0 || s->target_asset_id <= 0) {
        if (err_buf && err_len) {
            snprintf(err_buf, err_len, "source_asset_id and target_asset_id must be valid");
        }
        return -1;
    }
    if (!s->name[0]) {
        if (err_buf && err_len) {
            snprintf(err_buf, err_len, "Schedule name cannot be empty");
        }
        return -1;
    }
    if (!s->start_date[0]) {
        if (err_buf && err_len) {
            snprintf(err_buf, err_len, "start_date cannot be empty");
        }
        return -1;
    }
    if (!money_is_positive(s->expected_amount)) {
        if (err_buf && err_len) {
            snprintf(err_buf, err_len, "expected_amount must be positive");
        }
        return -1;
    }
    return 0;
...
int
mf_cashflow_rule_annual_factor(const char* freq, double* out_factor)
{
    if (!out_factor) {
        return -1;
    }
...
bool
mf_cashflow_rule_matches_month(const char* freq,
