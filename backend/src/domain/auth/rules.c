...
int mf_auth_rule_validate_username(const char* username, char* err_buf, size_t err_len) {
    if (!username || !username[0]) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "用户名不能为空");
        return -1;
    }
    size_t len = strlen(username);
    if (len < 2) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "用户名长度需大于等于 2 字符");
        return -1;
    }
    if (len > 64) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "用户名长度不能超过 64 字符");
        return -1;
    }
    return 0;
}
...
int mf_auth_rule_validate_password(const char* password, char* err_buf, size_t err_len) {
    if (!password || !password[0]) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "密码不能为空");
        return -1;
    }
    size_t len = strlen(password);
    if (len < 6) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "密码长度需大于等于 6 字符");
        return -1;
    }
    if (len > 128) {
        if (err_buf && err_len) snprintf(err_buf, err_len, "密码长度不能超过 128 字符");
        return -1;
    }
    return 0;
}
