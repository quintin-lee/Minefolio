           ...
           int64_t*         total)
{
    ...
    if (category_id && category_id[0]) {
        snprintf(
            ...
            sizeof(sql),
            "SELECT "
            "a.id,a.category_id,a.name,a.account_no,a.current_value,a.currency,a.note,a.created_at,"
...
asset_get(csilk_db_pool_t* pool, int64_t user_id, int64_t id)
{
    ...
    return csilk_db_query_param_json(
        pool,
        "SELECT "
        "a.id,a.category_id,a.name,a.account_no,a.current_value,a.currency,a.note,a.created_at,a."
                   ...
                   const char*      note)
{
    ...
    csilk_json_t* res =
        csilk_db_query_param_json(pool,
                                  "UPDATE assets SET "
                                  "name=?,account_no=?,current_value=?,currency=?,note=?,updated_"
                                  "at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?",
