...
int64_t tx_insert(csilk_db_pool_t* pool, int64_t user_id, int64_t asset_id, int64_t linked_asset_id,
                   int64_t category_id, const char* source_type, const char* transaction_type,
                   const char* direction, const char* linked_direction, double amount, double price_per_unit,
                   double quantity, double fee, const char* currency, const char* date, const char* note);
int tx_update(csilk_db_pool_t* pool, int64_t user_id, int64_t id, const char* transaction_type,
               const char* direction, const char* linked_direction, double amount, double price_per_unit,
               double quantity, const char* currency, const char* date, const char* note,
               int64_t category_id, const char* source_type, int64_t linked_asset_id);
