...
/**
 * @brief 将扁平分类列表构建为嵌套树结构
 *
 * 使用 yyjson 构建，属于表现层逻辑，保留在 controller 层。
 */
static csilk_json_t*
...
api_categories_list(csilk_ctx_t* c)
{
    ...
    category_usecase_seed_defaults(db_get_pool(), user_id);

    const char* type_query = csilk_get_query(c, "type");

    /* For multi-type queries (comma-separated), query each type and merge */
    if (type_query && strstr(type_query, ",")) {
        /* Split comma-separated types, query each, merge results */
        ...
        while (tok) {
            ...
            if (tok[0]) {
                ...
                if (category_usecase_list(db_get_pool(), user_id, tok, &part) == 0 && part) {
                    size_t pn = csilk_json_array_size(part);
                    for (size_t i = 0; i < pn; i++) {
                        csilk_json_add_item(merged, csilk_json_array_get(part, i));
                    }
                    /* Detach items to avoid double-free: just free the container */
    ...
    if (category_usecase_list(db_get_pool(), user_id, type_query, &list) != 0) {
        respond_error(c, 500, "查询分类列表失败");
        return;
    }

    csilk_json_t* tree = build_tree(list ? list : csilk_json_array());
    if (list) {
...
api_categories_children(csilk_ctx_t* c)
{
    ...
    csilk_json_t* list = NULL;
    if (category_usecase_children(db_get_pool(), user_id, atoll(id_str), &list) != 0) {
