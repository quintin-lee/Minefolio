                  ...
                  const char*         ctx_json)
{
    ...
    csilk_json_t* exp_stat = de_monthly_totals(pool, user_id, month);
    ...
    csilk_json_t* tx_stat = tx_monthly(pool, user_id, month);
    ...
    if (assets) {
        ...
        for (size_t i = 0; i < sz; i++) {
            ...
            if (type && (strcmp(type, "credit_card") == 0 || strcmp(type, "loan") == 0 ||
               ...
               const char*         ctx_json)
{
    ...
    csilk_json_t* cur_stat = de_monthly_totals(pool, user_id, month);
    csilk_json_t* prev_stat = de_monthly_totals(pool, user_id, prev_month);
    ...
    csilk_json_t* cat_stats = de_monthly_by_category(pool, user_id, month);
               ...
               const char*         ctx_json)
{
    ...
    if (income <= 0.0) {
        income = expense > 0 ? expense * 1.5 : 8000.0;
    }
                        ...
                        const char*         ctx_json)
{
    ...
    snprintf(buf,
             ...
             "3. "
             "**流动性保障**：应急备用金充足（%."
             "1f个月），建议多余闲置资金配置货币基金或稳健理财获取稳健收益。",
             month,
             score,
             inflows,
             expense,
               ...
               const char*         ctx_json)
{
    ...
    snprintf(
        ...
        fabs(dev) > 10.0 ? "⚠️ **偏离预警**：权益类资产偏离目标基准超过 "
                           "10%，建议通过逢高部分止盈或后续定投资金向固收类倾斜实现温和再平衡。"
