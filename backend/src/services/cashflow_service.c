...
cashflow_service_create_schedule(csilk_ctx_t* c)
{
    ...
    if (source_asset_id <= 0 || target_asset_id <= 0 || !name || !name[0] || !start_date || expected_amount <= 0.0) {
        respond_bad_request(c, "Missing required schedule fields (source_asset_id, target_asset_id, name, start_date, expected_amount)");
        return;
    }

    int64_t new_id = cashflow_schedule_create(pool, user_id, source_asset_id, target_asset_id,
                                              name, flow_type, frequency, start_date,
                                              end_date, expected_amount, note);
    if (new_id <= 0) {
...
cashflow_service_get_schedule(csilk_ctx_t* c)
{
    ...
    if (!res || csilk_json_array_size(res) == 0) {
        ...
        respond_not_found(c, "Schedule not found");
        return;
    }
    respond_ok(c, res);
}
...
cashflow_service_update_schedule(csilk_ctx_t* c)
{
    ...
    int ret = cashflow_schedule_update(pool, user_id, id, source_asset_id, target_asset_id,
                                       name, flow_type, frequency, start_date,
                                       end_date, expected_amount, note);
    if (ret != 0) {
...
cashflow_service_get_calendar(csilk_ctx_t* c)
{
    ...
    if (actual_txs) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            csilk_json_add_string(ev, "name", note && note[0] ? note : (asset_name ? asset_name : "收入"));
    ...
    if (schedules) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (!start_date) start_date = "2026-01-01";

            /* Calculate annual contribution */
            if (strcmp(freq, "monthly") == 0) annual_projected_total += expected_amount * 12.0;
            else if (strcmp(freq, "quarterly") == 0) annual_projected_total += expected_amount * 4.0;
            else if (strcmp(freq, "semi_annual") == 0) annual_projected_total += expected_amount * 2.0;
            else if (strcmp(freq, "annual") == 0 || strcmp(freq, "once") == 0) annual_projected_total += expected_amount;
            ...
            if (strcmp(freq, "once") == 0) {
                if (s_year == year && s_month == month) matches_this_month = true;
            } else if (strcmp(freq, "monthly") == 0) {
                if (year > s_year || (year == s_year && month >= s_month)) matches_this_month = true;
            } else if (strcmp(freq, "quarterly") == 0) {
                int total_months = (year - s_year) * 12 + (month - s_month);
                if (total_months >= 0 && (total_months % 3) == 0) matches_this_month = true;
            } else if (strcmp(freq, "semi_annual") == 0) {
                int total_months = (year - s_year) * 12 + (month - s_month);
                if (total_months >= 0 && (total_months % 6) == 0) matches_this_month = true;
            } else if (strcmp(freq, "annual") == 0) {
                if (year >= s_year && month == s_month) matches_this_month = true;
            }
    ...
    csilk_json_add_json(res, "events", events_arr);
...
cashflow_service_confirm(csilk_ctx_t* c)
{
    ...
    int64_t tx_id = tx_insert(pool, user_id, target_asset_id, source_asset_id,
                              0, /* category_id */
                              "income", "income", "in", "out",
                              amount, 0.0, 0.0, 0.0,
                              "CNY", full_date, tx_note);
    if (tx_id <= 0) {
