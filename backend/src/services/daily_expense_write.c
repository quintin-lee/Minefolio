...
void daily_expenses_create(csilk_ctx_t* c) {
    ...
    int64_t expense_id = de_insert(pool, user_id, category_id, asset_id, type, amount, currency, date, note);
    if (expense_id <= 0) {
...
void daily_expenses_update(csilk_ctx_t* c) {
    ...
    if (!de_update(pool, user_id, atoll(id_str), category_id, asset_id, type ? type : "", amount, currency ? currency : "CNY", date ? date : "", note ? note : "")) {
        csilk_db_exec(pool, "ROLLBACK");
        csilk_json_free(body);
        csilk_json_free(old_row);
