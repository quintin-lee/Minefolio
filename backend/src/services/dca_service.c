...
dca_service_create_plan(csilk_ctx_t* c)
{
    ...
    if (target_asset_id <= 0 || funding_asset_id <= 0 || !name || !name[0] || amount <= 0.0) {
        respond_bad_request(c, "Missing required plan fields (target_asset_id, funding_asset_id, name, amount)");
        return;
    }

    int64_t new_id = dca_plan_create(pool, user_id, target_asset_id, funding_asset_id,
                                      name, frequency, day_of_period, amount,
                                      target_profit_rate, target_total_amount,
                                      target_total_periods, note);
    if (new_id <= 0) {
...
dca_service_get_plan(csilk_ctx_t* c)
{
    ...
    if (!res || csilk_json_array_size(res) == 0) {
        ...
        respond_not_found(c, "DCA plan not found");
        return;
    }
    ...
    double total_invested = db_get_num(item, "total_invested_amount");
    double target_profit_rate = db_get_num(item, "target_profit_rate");
    double profit_rate = (total_invested > 0.0) ? (target_curr_val - total_invested) / total_invested : 0.0;
    bool profit_target_reached = (target_profit_rate > 0.0 && profit_rate >= target_profit_rate);
...
dca_service_update_plan(csilk_ctx_t* c)
{
    ...
    int ret = dca_plan_update(pool, user_id, id, target_asset_id, funding_asset_id,
                              name, frequency, day_of_period, amount,
                              target_profit_rate, target_total_amount,
                              target_total_periods, note);
    if (ret != 0) {
...
dca_service_confirm_execution(csilk_ctx_t* c)
{
    ...
    if (!exec_arr || csilk_json_array_size(exec_arr) == 0) {
        if (exec_arr) csilk_json_free(exec_arr);
        respond_not_found(c, "Execution task not found");
        return;
    }
    ...
    int64_t tx_id = tx_insert(pool, user_id, target_asset_id, funding_asset_id,
                              0, /* category_id */
                              "expense", "buy", "in", "out",
                              actual_amount, executed_price, executed_quantity,
                              0.0, /* fee */
                              "CNY", tx_date, "定投计划自动买入");
    if (tx_id <= 0) {
    ...
    apply_position(pool, target_asset_id, "buy", actual_amount, 0.0, executed_price, executed_quantity, &pos_delta);

    /* Debit funding account */
    balance_apply_delta(pool, funding_asset_id, user_id, -actual_amount, "transaction", tx_id, "定投扣款");

    /* Update execution record */
    dca_execution_update_confirmed(pool, exec_id, actual_amount, executed_price, executed_quantity, tx_id);
