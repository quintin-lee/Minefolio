...
static int
write_temp_file(const char* data, size_t len, const char* suffix, char* out_path, size_t out_path_len)
{
...
int
file_parse(csilk_db_pool_t* pool, const char* data, size_t data_len,
           const char* filename, char* out, size_t out_len)
{
    (void)pool;
...
char*
file_parse_to_string(csilk_db_pool_t* pool, const char* data, size_t data_len,
                     const char* filename, size_t max_len)
{
    if (!data || data_len == 0 || !filename) return NULL;
    if (max_len == 0) max_len = 50000;
