...
csilk_json_t*
mf_ai_session_get(csilk_db_pool_t* pool, int64_t user_id, int64_t id)
{
    char uid[32], id_s[32];
    uid_str(user_id, uid);
    sid_str(id, id_s);
...
int64_t
mf_ai_session_insert(csilk_db_pool_t* pool,
                     int64_t          user_id,
                     const char*      title,
                     const char*      model,
                     const char*      provider)
...
int
mf_ai_session_delete(csilk_db_pool_t* pool, int64_t user_id, int64_t id)
{
    char uid[32], id_s[32];
    uid_str(user_id, uid);
    sid_str(id, id_s);
...
int
mf_ai_message_delete_last_assistant(csilk_db_pool_t* pool, int64_t session_id)
{
    char sid[32];
...
csilk_json_t*
mf_ai_message_recent(csilk_db_pool_t* pool, int64_t session_id, int limit)
{
    char sid[32], lim[32];
    sid_str(session_id, sid);
...
int64_t
mf_ai_message_insert(csilk_db_pool_t* pool,
                     int64_t          session_id,
                     const char*      role,
                     const char*      content,
                     const char*      model)
