...
int mf_asset_repo_find_by_id(void* db_pool, int64_t user_id, int64_t id, mf_asset_t* out_asset) {
    ...
    if (name) snprintf(out_asset->name, sizeof(out_asset->name), "%s", name);

    const char* acc_no = csilk_json_get_string(row, "account_no");
    if (acc_no) snprintf(out_asset->account_no, sizeof(out_asset->account_no), "%s", acc_no);

    const char* atype = csilk_json_get_string(row, "asset_type");
    if (atype) snprintf(out_asset->asset_type, sizeof(out_asset->asset_type), "%s", atype);
    ...
    if (note) snprintf(out_asset->note, sizeof(out_asset->note), "%s", note);

    const char* sym = csilk_json_get_string(row, "symbol");
    if (sym) snprintf(out_asset->symbol, sizeof(out_asset->symbol), "%s", sym);

    const char* qs = csilk_json_get_string(row, "quote_source");
    if (qs) snprintf(out_asset->quote_source, sizeof(out_asset->quote_source), "%s", qs);

    money_from_double(db_get_num(row, "current_value"), out_asset->currency, &out_asset->current_value);
    ...
    if (cat) snprintf(out_asset->created_at, sizeof(out_asset->created_at), "%s", cat);

    const char* uat = csilk_json_get_string(row, "updated_at");
    if (uat) snprintf(out_asset->updated_at, sizeof(out_asset->updated_at), "%s", uat);
...
int mf_asset_repo_save(void* db_pool, const mf_asset_t* asset, int64_t* out_id) {
    if (!asset || !db_pool) return -1;
    char cur_str[8];
    currency_to_str(asset->currency, cur_str, sizeof(cur_str));
...
int mf_asset_repo_update_position(void* db_pool, int64_t user_id, int64_t id,
    ...
    return asset_update_position((csilk_db_pool_t*)db_pool,
                                 ...
                                 money_to_double(cost_basis)) ? 0 : -1;
}
