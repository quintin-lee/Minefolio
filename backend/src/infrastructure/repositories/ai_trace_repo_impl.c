...
csilk_json_t*
mf_ai_trace_list(csilk_db_pool_t* pool,
                 int64_t          user_id,
                 int64_t          page,
                 int64_t          page_size,
                 const char*      provider,
                 ...
                 int64_t*         total)
{
    ...
    *total = 0;
    ...
    CSILK_LOG_I("mf_ai_trace_list: user_id=%lld page=%lld page_size=%lld total=%lld",
                (long long)user_id,
                (long long)page,
                (long long)page_size,
                (long long)*total);
...
csilk_json_t*
mf_ai_trace_get(csilk_db_pool_t* pool, int64_t user_id, int64_t id)
{
    char uid[32], id_s[32];
    uid_str(user_id, uid);
    snprintf(id_s, 32, "%lld", (long long)id);

    CSILK_LOG_I("mf_ai_trace_get: user_id=%lld id=%lld", (long long)user_id, (long long)id);
    ...
    if (!r || csilk_json_array_size(r) == 0) {
        CSILK_LOG_W(
            "mf_ai_trace_get: not found user_id=%lld id=%lld", (long long)user_id, (long long)id);
        csilk_json_free(r);
        return NULL;
    }
    return r;
...
csilk_json_t*
mf_ai_trace_stats(csilk_db_pool_t* pool, int64_t user_id)
{
    char uid[32];
    uid_str(user_id, uid);

    CSILK_LOG_I("mf_ai_trace_stats: user_id=%lld", (long long)user_id);
...
int64_t
mf_ai_trace_save(csilk_db_pool_t* pool, ai_trace_t* t)
{
    ...
    if (t->tool_spans[0]) {
        ...
        if (spans) {
            csilk_json_add_object(meta_obj, "tool_spans", spans);
        }
    }
    ...
    CSILK_LOG_I("mf_ai_trace_save: user_id=%lld model='%s' provider='%s' status='%s' tokens=%d",
                (long long)t->user_id,
                t->model,
                t->provider,
                t->status,
    ...
    CSILK_LOG_I("mf_ai_trace_save: id=%lld", (long long)id);
    csilk_json_free(r);
    free(meta_final);
    return id;
}
