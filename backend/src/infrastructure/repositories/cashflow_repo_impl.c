...
int mf_cashflow_repo_list(void* pool, int64_t user_id, mf_cashflow_schedule_t** out_list, size_t* out_count) {
    if (!pool || !out_list || !out_count) return -1;
    *out_list = NULL;
    *out_count = 0;
    ...
    mf_cashflow_schedule_t* arr = (mf_cashflow_schedule_t*)calloc(n, sizeof(mf_cashflow_schedule_t));
    if (!arr) {
        csilk_json_free(json);
        return -1;
    }
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (s) snprintf(arr[i].name, sizeof(arr[i].name), "%s", s);
        s = csilk_json_get_string(row, "flow_type");
        if (s) snprintf(arr[i].flow_type, sizeof(arr[i].flow_type), "%s", s);
        s = csilk_json_get_string(row, "frequency");
        if (s) snprintf(arr[i].frequency, sizeof(arr[i].frequency), "%s", s);
        s = csilk_json_get_string(row, "start_date");
        if (s) snprintf(arr[i].start_date, sizeof(arr[i].start_date), "%s", s);
        s = csilk_json_get_string(row, "end_date");
        if (s) snprintf(arr[i].end_date, sizeof(arr[i].end_date), "%s", s);
        s = csilk_json_get_string(row, "note");
        if (s) snprintf(arr[i].note, sizeof(arr[i].note), "%s", s);
        s = csilk_json_get_string(row, "status");
        if (s) snprintf(arr[i].status, sizeof(arr[i].status), "%s", s);
...
int mf_cashflow_repo_get(void* pool, int64_t user_id, int64_t id, mf_cashflow_schedule_t* out_schedule) {
    if (!pool || !out_schedule || id <= 0) return -1;
    ...
    const char* s = csilk_json_get_string(row, "name");
    if (s) snprintf(out_schedule->name, sizeof(out_schedule->name), "%s", s);
    s = csilk_json_get_string(row, "flow_type");
    if (s) snprintf(out_schedule->flow_type, sizeof(out_schedule->flow_type), "%s", s);
    s = csilk_json_get_string(row, "frequency");
    if (s) snprintf(out_schedule->frequency, sizeof(out_schedule->frequency), "%s", s);
    s = csilk_json_get_string(row, "start_date");
    if (s) snprintf(out_schedule->start_date, sizeof(out_schedule->start_date), "%s", s);
    s = csilk_json_get_string(row, "end_date");
    if (s) snprintf(out_schedule->end_date, sizeof(out_schedule->end_date), "%s", s);
    s = csilk_json_get_string(row, "note");
    if (s) snprintf(out_schedule->note, sizeof(out_schedule->note), "%s", s);
    s = csilk_json_get_string(row, "status");
    if (s) snprintf(out_schedule->status, sizeof(out_schedule->status), "%s", s);
...
int mf_cashflow_repo_create(void* pool, const mf_cashflow_schedule_t* s, int64_t* out_id) {
    if (!pool || !s) return -1;
...
int mf_cashflow_repo_update(void* pool, const mf_cashflow_schedule_t* s) {
    if (!pool || !s || s->id <= 0) return -1;
...
int mf_cashflow_repo_get_actual_events(void* pool, int64_t user_id, const char* year_month,
    ...
    if (!pool || !out_events || !out_count) return -1;
    *out_events = NULL;
    *out_count = 0;

    csilk_json_t* actual_txs = cashflow_query_actual_transactions((csilk_db_pool_t*)pool, user_id, year_month);
    if (!actual_txs) return 0;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        snprintf(evs[i].name, sizeof(evs[i].name), "%s",
