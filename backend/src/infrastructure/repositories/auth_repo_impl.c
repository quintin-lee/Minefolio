...
int mf_auth_repo_find_by_username(void* pool, const char* username, mf_user_t* out_user) {
    if (!pool || !username || !username[0] || !out_user) return -1;
    ...
    if (s) snprintf(out_user->username, sizeof(out_user->username), "%s", s);
    s = csilk_json_get_string(row, "password_hash");
    if (s) snprintf(out_user->password_hash, sizeof(out_user->password_hash), "%s", s);
    ...
    if (s) snprintf(out_user->totp_secret, sizeof(out_user->totp_secret), "%s", s);
    s = csilk_json_get_string(row, "created_at");
    if (s) snprintf(out_user->created_at, sizeof(out_user->created_at), "%s", s);
...
int mf_auth_repo_get_by_id(void* pool, int64_t user_id, mf_user_t* out_user) {
    if (!pool || user_id <= 0 || !out_user) return -1;
    memset(out_user, 0, sizeof(*out_user));
...
int mf_auth_repo_create(void* pool, const char* username, const char* password_hash, int64_t* out_id) {
    if (!pool || !username || !password_hash) return -1;
...
int mf_auth_repo_update_password(void* pool, int64_t user_id, const char* password_hash) {
    if (!pool || user_id <= 0 || !password_hash) return -1;
