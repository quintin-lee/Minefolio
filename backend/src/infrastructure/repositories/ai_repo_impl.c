...
int mf_ai_session_repo_list(void* pool, int64_t user_id, int64_t page, int64_t page_size,
    ...
    if (out_list && max_out > 0) {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (s) snprintf(out_list[i].title, sizeof(out_list[i].title), "%s", s);
            s = csilk_json_get_string(row, "model");
            if (s) snprintf(out_list[i].model, sizeof(out_list[i].model), "%s", s);
            s = csilk_json_get_string(row, "provider");
            if (s) snprintf(out_list[i].provider, sizeof(out_list[i].provider), "%s", s);
            s = csilk_json_get_string(row, "created_at");
            if (s) snprintf(out_list[i].created_at, sizeof(out_list[i].created_at), "%s", s);
            s = csilk_json_get_string(row, "updated_at");
            if (s) snprintf(out_list[i].updated_at, sizeof(out_list[i].updated_at), "%s", s);
        }
    }
...
int mf_ai_session_repo_get(void* pool, int64_t user_id, int64_t id, mf_ai_session_t* out_session) {
    if (!pool || user_id <= 0 || id <= 0 || !out_session) return -1;
    ...
    if (s) snprintf(out_session->title, sizeof(out_session->title), "%s", s);
    s = csilk_json_get_string(res, "model");
    if (s) snprintf(out_session->model, sizeof(out_session->model), "%s", s);
    s = csilk_json_get_string(res, "provider");
    if (s) snprintf(out_session->provider, sizeof(out_session->provider), "%s", s);
    s = csilk_json_get_string(res, "created_at");
    if (s) snprintf(out_session->created_at, sizeof(out_session->created_at), "%s", s);
    s = csilk_json_get_string(res, "updated_at");
    if (s) snprintf(out_session->updated_at, sizeof(out_session->updated_at), "%s", s);
...
int mf_ai_session_repo_update(void* pool, int64_t user_id, int64_t id,
    ...
    if (!pool || user_id <= 0 || id <= 0) return 0;
...
int mf_ai_trace_repo_stats(void* pool, int64_t user_id, mf_ai_trace_summary_t* out_stats) {
    if (!pool || user_id <= 0 || !out_stats) return -1;
