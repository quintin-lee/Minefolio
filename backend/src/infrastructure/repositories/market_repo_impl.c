    ...
    void* pool, int64_t asset_id, const char* date, price_t price, currency_t cur)
{
    ...
    return price_history_record(
        (csilk_db_pool_t*)pool, asset_id, date, price_to_double(price), currency_code(&cur));
}
...
mf_market_repo_update_asset_quote(void* pool, int64_t user_id, int64_t asset_id, price_t price)
{
    ...
    return asset_update_market_quote(
        (csilk_db_pool_t*)pool, user_id, asset_id, price_to_double(price));
}
